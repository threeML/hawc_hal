...
from conf_test import check_map_trees, check_responses
...
def test_root_to_hdf_response():
    ...
    root_response = os.path.join(test_data_path, "response.root")

    r = hawc_response_factory(root_response)
    ...
    orig_par, orig_like = test_on_point_source(ra=ra_mkn421, dec=dec_mkn421,
                                               response=root_response, data_radius=data_radius,
