...
def test_fast_bilinear_interpolation():
    ...
    mfi = fast_bilinar_interpolation.FastBilinearInterpolation(
        data.shape, (new_coords[:, 0], new_coords[:, 1])
    )

    v1 = mfi(data)

    # Check against the slower scipy interpolator in map_coordinates
    v2 = scipy.ndimage.map_coordinates(
        data, np.array((new_coords[:, 0], new_coords[:, 1])), order=1
    )
